...
const verbsEndMatch = content.lastIndexOf('];');
const beforeVerbsEnd = content.substring(0, verbsEndMatch);
const afterVerbsEnd = content.substring(verbsEndMatch);

// Find existing verbs to avoid duplicates
const existingVerbsMatch = content.match(/infinitive: "([^"]+)"/g);
...
fs.writeFileSync('./src/data/dutchData.ts', beforeVerbsEnd + newVerbsStr + '\n' + afterVerbsEnd);
console.log(`Added ${filteredNewVerbs.length} new verbs.`);
